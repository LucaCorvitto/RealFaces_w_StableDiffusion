        ...
        for phase in ['train', 'eval']:
            ...
            if phase == 'eval':
                ...
                if epoch_acc > best_acc:
                    ...
                    best_acc = epoch_acc
                    torch.save(model, 'work/project/5_classes_results/' + model_name + '/' + transform_type + '_' + '_' + "best.pth")
                else: #early stopping
                    count -= 1
                    early_stopping -= 1
                    if count == 0:
...
def save_all(model_name, history, transform_type):
    ...
    path_train = 'work/project/5_classes_results/' + model_name + '/train_' + transform_type + '_' + '.txt'
    path_eval = 'work/project/5_classes_results/' + model_name + '/eval_' + transform_type + '_' + '.txt'
    path_test = 'work/project/5_classes_results/' + model_name + '/test_' + transform_type + '_' + '.txt'
...
def create_dir(path):
    if not os.path.exists('work/project/5_classes_results/' + path):
        os.mkdir('work/project/5_classes_results/' + path)
        print(f'created folder: work/project/5_classes_results/{path}')
    else:
        print(f'directory work/project/5_classes_results/{path} already exists')
...
def main():
    ...
    #parameters
    data_dir = 'work/project/datasets/5_classes'
    input_size = 224
    workers = 6
