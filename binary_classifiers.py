...
def train_model(model, model_name, best_acc, dataloaders, criterion, optimizer, transform_type, compression_type, count=7, early_stopping=24, num_epochs=25):
        ...
        for phase in ['train', 'eval']:
            ...
            if phase == 'eval':
                ...
                if epoch_acc > best_acc:
                    ...
                    best_acc = epoch_acc
                    torch.save(model, model_name + '/' + transform_type + '_' + compression_type + '_' + "best.pth")
                else: #early stopping
                    count -= 1
                    early_stopping -= 1
                    if count == 0:
...
def initialize_model(model_name):
    ...
    #   Resnet50
        model_ft = timm.create_model('resnet50', pretrained=True, num_classes=2) # change when classifying GAN also 
        input_size = 224

    elif model_name == "mobilenet":
    #   MobileNetv2
        model_ft = timm.create_model('mobilenetv2_100', pretrained=True, num_classes=2)
        input_size = 224
    ...
    #   Xception
        model_ft = timm.create_model('xception', pretrained=True, num_classes=2)
        input_size = 224
...
def save_all(model_name, history, transform_type, compression_type):
    
    path_train = model_name + '/train_' + transform_type + '_' + compression_type + '.txt'
    path_eval = model_name + '/eval_' + transform_type + '_' + compression_type + '.txt'
    path_test = model_name + '/test_' + transform_type + '_' + compression_type + '.txt'
...
def compress_create():
    dataset_dirs = ['train','eval','test']
    for quality in [100,90,70]:
        create_dir(f'datasets/jpg{quality}_images') # create folder jpg_100, jpg90, jpg70 in datasets
        for dir in dataset_dirs:
            create_dir(f'datasets/jpg{quality}_images/{dir}') # create train, eval and test in datasets/jpg
            for folder in ['fake','real']:
                create_dir(f'datasets/jpg{quality}_images/{dir}/{folder}/') # create fake and real in each datasets/jpg/train-test-eval
    for dir in dataset_dirs:
        for folder in ['fake','real']:
            path = f'datasets/png_images/{dir}/{folder}/' # access to the original images 
...
def main():
    ...
    for type, t_type in enumerate(transform_type):
        ...
        for data_dir,compression_type in data_dirs:
            ...
            for model_name in model_names:
                ...
                # Train and evaluate
                model_ft, hist, best_acc = train_model(model_ft, model_name, best_acc, dataloaders_dict, criterion, optimizer_ft, t_type, compression_type, num_epochs=num_epochs)

                #evaluate the model on the test set
                hist["test_acc"] = evaluate(model_ft,dataloaders_dict['test'])
                

                save_all(model_name, hist, t_type, compression_type)
